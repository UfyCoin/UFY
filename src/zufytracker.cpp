...
CzUFYTracker::CzUFYTracker(std::string strWalletFile)
{
...
CzUFYTracker::~CzUFYTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzUFYTracker::Init()
{
...
bool CzUFYTracker::Archive(CMintMeta& meta)
{
...
bool CzUFYTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzUFYTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzUFYTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzUFYTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzUFYTracker::GetSerialHashes()
{
...
CAmount CzUFYTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzUFYTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzUFYTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzUFYTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzUFYTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzUFYTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzUFYTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzUFYTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzUFYTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzUFYTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzUFYTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzUFYWallet* zUFYWallet)
{
    bool iszUFYWalletInitialized = (NULL != zUFYWallet);
    CMintMeta meta;
    ...
    if (! iszUFYWalletInitialized)
        zUFYWallet = new CzUFYWallet(strWalletFile);
    meta.isSeedCorrect = zUFYWallet->CheckSeed(dMint);
    if (! iszUFYWalletInitialized)
        delete zUFYWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzUFYTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzUFYTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzUFYTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzUFYTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzUFYTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzUFYTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzUFYWallet* zUFYWallet = new CzUFYWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zUFYWallet);
        }
        delete zUFYWallet;
        LogPrint("zero", "%s: added %d dzufy from DB\n", __func__, listDeterministicDB.size());
    }
