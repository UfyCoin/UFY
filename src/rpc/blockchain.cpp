...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zufyObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zufyObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "  \"zUFYsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zUFY denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zUFY denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zUFY denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zUFY denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zUFY denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zUFY denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zUFY denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zUFY denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zUFY denominations\n"
            "  }\n"
            "}\n"
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in ufy\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ufy addresses\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, 100, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZUFY_FAILED_ACCUMULATOR_INITIALIZATION);
