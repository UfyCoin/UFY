...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 UFY at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zUFY send
enum ZerocoinSpendStatus {
    ZUFY_SPEND_OKAY = 0,                            // No error
    ZUFY_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZUFY_WALLET_LOCKED = 2,                         // Wallet was locked
    ZUFY_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZUFY_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZUFY_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZUFY_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZUFY_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZUFY_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZUFY_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZUFY_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZUFY_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZUFY_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZUFY_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZUFY_SPENT_USED_ZUFY = 14,                      // Coin has already been spend
    ZUFY_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZUFY_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZUfyBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZUFYOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzufyAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzUFYTracker> zufyTracker;
    ...
    void setZWallet(CzUFYWallet* zwallet)
    {
        zwalletMain = zwallet;
        zufyTracker = std::unique_ptr<CzUFYTracker>(new CzUFYTracker(strWalletFile));
    }

    CzUFYWallet* getZWallet() { return zwalletMain; }
    ...
    void setZUfyAutoBackups(bool fEnabled)
    {
    ...
    /** zUFY reset */
    boost::signals2::signal<void()> NotifyzUFYReset;
