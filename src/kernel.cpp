...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZUfyStake(spend));
    } else {
        ...
        CUfyStake* ufyInput = new CUfyStake();
        ufyInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(ufyInput);
    }
