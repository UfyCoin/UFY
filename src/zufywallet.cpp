...
CzUFYWallet::CzUFYWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zufy seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZUFYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZUFYSeed_deprecated()) {
                    LogPrintf("%s: Updated zUFY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zufy seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zufy wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzUFYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZUFYCount(nCountLastUsed);
    else if (!walletdb.ReadZUFYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzUFYWallet::Lock()
{
    seedMaster = 0;
}

void CzUFYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzUFYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZUFY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzUFYWallet::LoadMintPoolFromDB()
{
...
void CzUFYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzUFYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzUFYWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zufyTracker->HasPubcoinHash(pMint.first)) {
...
bool CzUFYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZUFY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zufyTracker which also adds to database
    pwalletMain->zufyTracker->Add(dMint, true);
...
void CzUFYWallet::SeedToZUFY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzUFYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzUFYWallet::GenerateDeterministicZUFY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzUFYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzUFYWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzUFYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
