...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::UFY);
    ...
    if (!settings.contains("nAnonymizeUnifyCoinAmount"))
        settings.setValue("nAnonymizeUnifyCoinAmount", 1000);

    nAnonymizeUnifyCoinAmount = settings.value("nAnonymizeUnifyCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeUnifyCoinAmount"))
        SoftSetArg("-anonymizeufyamount", settings.value("nAnonymizeUnifyCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ufy.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeUfyAmount:
            return QVariant(nAnonymizeUnifyCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeUfyAmount:
            nAnonymizeUnifyCoinAmount = value.toInt();
            settings.setValue("nAnonymizeUnifyCoinAmount", nAnonymizeUnifyCoinAmount);
            emit anonymizeUfyAmountChanged(nAnonymizeUnifyCoinAmount);
            break;
        case CoinControlFeatures:
