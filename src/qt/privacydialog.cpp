                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zUFY ought to be enough for anybody." - Bill Gates, 2017
    ui->zUFYpayAmount->setValidator( new QDoubleValidator(0.0, 500000000.0, 20, this) );
    ...
    ui->labelzUFYSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zUFYpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzUFY_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUFY is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zUFY, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zUFY...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zUFY in ") +
...
void PrivacyDialog::on_pushButtonSpendzUFY_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zUFY is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zUFY, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zUFY
        sendzUFY();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zUFY
    sendzUFY();
}

void PrivacyDialog::on_pushButtonZUfyControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZUfyControlDialog* zUfyControl = new ZUfyControlDialog(this);
    zUfyControl->setModel(walletModel);
    zUfyControl->exec();
}

void PrivacyDialog::setZUfyControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzUfySelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzUFY()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid UnifyCoin address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zUFYpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zUFY is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " UFY </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zUFY</b>";
    ...
    // use mints from zUFY selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZUfyControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZUfyControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zUFY require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zUFY"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZUFY_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zUFY require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zUFY"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zUFY was spent successfully update the addressbook with the label
    ...
    // Clear zufy selector in case it was used
    ZUfyControlDialog::setSelectedMints.clear();
    ui->labelzUfySelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zUFY Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " UFY, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zUFY Mint");
                               ...
                               const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zufyTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zUFY </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zUFY "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zUFY "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zUFY "));
    ui->labelzUFYAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zUFY </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zUFY </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zUFY </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzUFYSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzUFY->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zUFY
        ui->pushButtonMintzUFY->setEnabled(false);
        ui->pushButtonMintzUFY->setToolTip(tr("zUFY is currently disabled due to maintenance."));

        // Spend zUFY
        ui->pushButtonSpendzUFY->setEnabled(false);
        ui->pushButtonSpendzUFY->setToolTip(tr("zUFY is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zUFY
        ui->pushButtonMintzUFY->setEnabled(true);
        ui->pushButtonMintzUFY->setToolTip(tr("PrivacyDialog", "Enter an amount of UFY to convert to zUFY", 0));

        // Spend zUFY
        ui->pushButtonSpendzUFY->setEnabled(true);
        ui->pushButtonSpendzUFY->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
