...
#include "ui_zufycontroldialog.h"
...
std::set<std::string> ZUfyControlDialog::setSelectedMints;
std::set<CMintMeta> ZUfyControlDialog::setMints;

bool CZUfyControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZUfyControlDialog::COLUMN_DENOMINATION || column == ZUfyControlDialog::COLUMN_VERSION || column == ZUfyControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZUfyControlDialog::ZUfyControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZUfyControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZUfyControlDialog::~ZUfyControlDialog()
{
    delete ui;
}

void ZUfyControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUfyControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZUfyControlWidgetItem* itemDenom(new CZUfyControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZUfyControlWidgetItem *itemMint = new CZUfyControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zUFY seed used to mint this zUFY is not the same as currently hold in the wallet";
            else
...
void ZUfyControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUfyControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZUfy_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUfyControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZUfyControlDialog::GetSelectedMints()
{
...
void ZUfyControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
