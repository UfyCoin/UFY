...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::UFY)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sUFYPercentage, QString& szUFYPercentage)
{
    int nPrecision = 2;
    ...
    szUFYPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sUFYPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& earnings, const CAmount& masternodeEarnings, const CAmount& stakeEarnings)
{
    ...
    CAmount ufyAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // UFY Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = ufyAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // UFY labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, ufyAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    QString automintHelp = tr("Current percentage of zUFY.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", Params().ZeroCoinEnabled());
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in ufy.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in ufy.conf");
    }
    ...
    // UFY Available
    bool showUFYAvailable = settingShowAllBalances || ufyAvailableBalance != nTotalBalance;
    bool showWatchOnlyUFYAvailable = showUFYAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showUFYAvailable || showWatchOnlyUFYAvailable);
    ui->labelBalance->setVisible(showUFYAvailable || showWatchOnlyUFYAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlyUFYAvailable && showWatchOnly);

    // UFY Pending
    bool showUFYPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyUFYPending = showUFYPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showUFYPending || showWatchOnlyUFYPending);
    ui->labelUnconfirmed->setVisible(showUFYPending || showWatchOnlyUFYPending);
    ui->labelWatchPending->setVisible(showWatchOnlyUFYPending && showWatchOnly);

    // UFY Immature
    bool showUFYImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showUFYImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showUFYImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showUFYImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // UFY Locked
    bool showUFYLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyUFYLocked = showUFYLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showUFYLocked || showWatchOnlyUFYLocked);
    ui->labelLockedBalance->setVisible(showUFYLocked || showWatchOnlyUFYLocked);
    ui->labelWatchLocked->setVisible(showWatchOnlyUFYLocked && showWatchOnly);
    ...
    ui->labelUFYPercent->setVisible(false);
    ui->labelzUFYPercent->setVisible(false);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("UFY")
    updateDisplayUnit();
