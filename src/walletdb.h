...
public:
    ...
    bool WriteZUFYSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZUFYSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZUFYSeed_deprecated(uint256& seed);
    bool EraseZUFYSeed();
    bool EraseZUFYSeed_deprecated();

    bool WriteZUFYCount(const uint32_t& nCount);
    bool ReadZUFYCount(uint32_t& nCount);
